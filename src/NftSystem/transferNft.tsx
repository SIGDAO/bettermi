...
async function getNftList(ledger2:Api,nftStorageAccount:string,nftDistributor:string){
    ...
    for(var i = message.unconfirmedTransactions.length - 1;i >= 0;i--){
        if(message.unconfirmedTransactions[i].recipient === nftStorageAccount && message.unconfirmedTransactions[i].sender === nftDistributor){  //Also need to check sender
...
export async function SendBackToStorage(ledger2:any,nftToBeReturned:string,nftDistributor:string,nftStorageAccount:string,
    nftDistributorPublicKey:string,nftDistributorPrivateKey:string,recipientPrivateKey:string,recipientPublicKey:string){
    ...
    let ourContract = await ledger2.contract.getContractsByAccount({
      ...
      const transaction =  await ledger2.account.getUnconfirmedAccountTransactions(ourContract.ats[0].at);
        ...
        for(var i = unconfirmedTransactionList.length - 1;i >= 0;i--){
            if(unconfirmedTransactionList[i].recipient === ourContract && unconfirmedTransactionList[i].sender === nftDistributor){  //Also need to check sender
...
async function CheckAndOpenNftContract(ledger2:any,recipient:string,codeHashId:string){
    let ourContract = await ledger2.contract.getContractsByAccount({
      ...
      if(ourContract.ats[0] == null){
        setTimeout(async() => {const hi = await CheckAndOpenNftContract(ledger2,recipient,codeHashId);return hi}, 10000);
      }
      else{
        return true;
      }
...
export const TransferNft = async (ledger2:any,recipientId:string,nftStorageAccounts:string[],codeHashId:string,nftDistributor:string,nftDistributorPublicKey:string,nftDistributorPrivateKey:string) => {
