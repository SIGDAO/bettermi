...
export async function sendMessage(ledger2:any,message:string,recipientId:string,senderPublicKey:string,senderPrivateKey:string,feePlanck:string){
...
export async function UpdateUserNftList(ledger2:any,nftToBeDistributed:string,recipient:string,codeHashId:string,nftDistributor:string,nftDistributorPublicKey:string,nftDistributorPrivateKey:string){
    let ourContract = await ledger2.contract.getContractsByAccount({
      ...
      const transaction =  await ledger2.account.getUnconfirmedAccountTransactions(ourContract.ats[0].at);
        ...
        for(var i = unconfirmedTransactionList.length - 1;i >= 0;i--){
            if(unconfirmedTransactionList[i].recipient === ourContract.ats[0].at && unconfirmedTransactionList[i].sender === nftDistributor){  //Also need to check sender
